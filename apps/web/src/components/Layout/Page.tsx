...
export const PageMeta: React.FC<React.PropsWithChildren<{ symbol?: string }>> = ({ symbol }) => {
  ...
  const dfsPriceUsd = useDFSBusdPrice({ forceMainnet: false })
  const dfsPriceUsdDisplay = dfsPriceUsd ? `$${dfsPriceUsd.toFixed(3)}` : '...'

  const pageMeta = getCustomMeta(pathname, t, locale) || {}
  const { title, description, image } = { ...DEFAULT_META, ...pageMeta }
  let pageTitle = dfsPriceUsdDisplay ? [title, dfsPriceUsdDisplay].join(' - ') : title
  if (symbol) {
